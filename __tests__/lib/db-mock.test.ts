...
describe("db-mock", () => {
  ...
  describe("query operators", () => {
    it("should support $lte operator", async () => {
      ...
      const matchingItems = results.filter((r) => (r as any).score <= 20);
      expect(matchingItems).toHaveLength(2);
      expect(matchingItems.map((r) => (r as any).name)).toContain("item1");
      expect(matchingItems.map((r) => (r as any).name)).toContain("item2");
    });
    ...
    it("should support $gte operator", async () => {
      ...
      expect(matchingItems).toHaveLength(2);
      expect(matchingItems.map((r) => (r as any).name)).toContain("item2");
      expect(matchingItems.map((r) => (r as any).name)).toContain("item3");
    });
    ...
    it("should support $lt operator", async () => {
      ...
      const matchingItems = results.filter((r) => (r as any).score < 25);
      expect(matchingItems).toHaveLength(2);
      expect(matchingItems.map((r) => (r as any).name)).toContain("item1");
      expect(matchingItems.map((r) => (r as any).name)).toContain("item2");
    });
    ...
    it("should support $gt operator", async () => {
      ...
      const matchingItems = results.filter((r) => (r as any).score > 15);
      expect(matchingItems).toHaveLength(2);
      expect(matchingItems.map((r) => (r as any).name)).toContain("item2");
      expect(matchingItems.map((r) => (r as any).name)).toContain("item3");
    });
    ...
    it("should support combined comparison operators", async () => {
      ...
      expect((matchingItems[0] as any).name).toBe("item2");
    });
  });
