...
describe("permissions", () => {
  ...
  describe("checkModulePermission", () => {
    describe("admin access", () => {
      it("should grant admin full access regardless of role", () => {
        ...
        expect(
          checkModulePermission(mockModule, "any-user-id", true, "owner"),
        ).toBe(true);
        expect(
          checkModulePermission(mockModule, "any-user-id", true, "write"),
        ).toBe(true);
        expect(
          checkModulePermission(mockModule, "any-user-id", true, "read"),
        ).toBe(true);
      });
    });
    ...
    describe("owner access", () => {
      it("should grant owner full access", () => {
        const mockModule = createMockModule();
        expect(
          checkModulePermission(mockModule, "owner-id", false, "owner"),
        ).toBe(true);
        expect(
          checkModulePermission(mockModule, "owner-id", false, "write"),
        ).toBe(true);
        expect(
          checkModulePermission(mockModule, "owner-id", false, "read"),
        ).toBe(true);
      });
      ...
      it("should deny non-owner for owner-level permission", () => {
        const mockModule = createMockModule();
        expect(
          checkModulePermission(mockModule, "other-user", false, "owner"),
        ).toBe(false);
      });
    });
    ...
    describe("maintainer access - write role", () => {
      it("should grant write access to admin maintainers", () => {
        ...
        expect(
          checkModulePermission(mockModule, "maintainer-id", false, "write"),
        ).toBe(true);
      });
      ...
      it("should deny write access to non-maintainers", () => {
        const mockModule = createMockModule();
        expect(
          checkModulePermission(mockModule, "random-user", false, "write"),
        ).toBe(false);
      });
    });
    ...
    describe("maintainer access - read role", () => {
      it("should grant read access to admin maintainers", () => {
        ...
        expect(
          checkModulePermission(mockModule, "maintainer-id", false, "read"),
        ).toBe(true);
      });
      ...
      it("should deny read access to non-maintainers", () => {
        const mockModule = createMockModule();
        expect(
          checkModulePermission(mockModule, "random-user", false, "read"),
        ).toBe(false);
      });
    });
    ...
    describe("multiple maintainers", () => {
      it("should find correct user in maintainer list", () => {
        ...
        expect(checkModulePermission(mockModule, "user-1", false, "read")).toBe(
          true,
        );
        expect(
          checkModulePermission(mockModule, "user-1", false, "write"),
        ).toBe(false);

        expect(
          checkModulePermission(mockModule, "user-2", false, "write"),
        ).toBe(true);
        expect(checkModulePermission(mockModule, "user-2", false, "read")).toBe(
          true,
        );

        expect(
          checkModulePermission(mockModule, "user-3", false, "write"),
        ).toBe(true);
        expect(checkModulePermission(mockModule, "user-3", false, "read")).toBe(
          true,
        );
      });
    });
    ...
    describe("edge cases", () => {
      it("should handle modules without maintainers array", () => {
        ...
        expect(
          checkModulePermission(mockModule, "user-id", false, "write"),
        ).toBeFalsy();
        expect(
          checkModulePermission(mockModule, "user-id", false, "read"),
        ).toBeFalsy();
      });
      ...
      it("should default to write permission when no role specified", () => {
        ...
        expect(checkModulePermission(mockModule, "maintainer-id", false)).toBe(
          true,
        );
      });
    });
  ...
  describe("canPublishVersion", () => {
    it("should have same permissions as canApproveSubscription", () => {
      ...
      expect(canPublishVersion(adminSession, mockModule)).toBe(
        canApproveSubscription(adminSession, mockModule),
      );
      expect(canPublishVersion(userSession, mockModule)).toBe(
        canApproveSubscription(userSession, mockModule),
      );
      expect(canPublishVersion(null, mockModule)).toBe(
        canApproveSubscription(null, mockModule),
      );
    });
  ...
  describe("permission hierarchy", () => {
    it("should follow the permission hierarchy: admin > owner > write > read", () => {
      ...
      const adminMaintainerSession = createMockSession({
        id: "admin-maintainer",
      });
      const writeMaintainerSession = createMockSession({
        id: "write-maintainer",
      });
      const readMaintainerSession = createMockSession({
        id: "read-maintainer",
      });
