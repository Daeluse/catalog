...
describe('validateOrigins', () => {
  it('should validate correct HTTP origins', () => {
    const result = validateOrigins(['http://localhost:3000', 'http://example.com'])

    expect(result.valid).toEqual(['http://localhost:3000', 'http://example.com'])
    expect(result.invalid).toEqual([])
  })

  it('should validate correct HTTPS origins', () => {
    const result = validateOrigins(['https://example.com', 'https://www.example.com'])

    expect(result.valid).toEqual(['https://example.com', 'https://www.example.com'])
    expect(result.invalid).toEqual([])
  })

  it('should accept localhost with port', () => {
    const result = validateOrigins(['http://localhost:3000', 'http://localhost:8080'])
  ...
  it('should reject invalid URLs', () => {
    ...
    expect(result.invalid[0].reason).toBe('Must be a valid http:// or https:// URL')
    expect(result.invalid[1].origin).toBe('ftp://example.com')
    expect(result.invalid[1].reason).toBe('Must be a valid http:// or https:// URL')
  })
  ...
  it('should handle mixed valid and invalid origins', () => {
    ...
    expect(result.valid).toEqual(['https://example.com', 'http://localhost:3000'])
