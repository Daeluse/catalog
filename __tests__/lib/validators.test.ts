...
describe("validators", () => {
  ...
  describe("origin", () => {
    ...
    it("should reject origin URLs with paths", () => {
      ...
      expect(validators.origin("https://example.com/path/to/resource")).toBe(
        false,
      );
    });
...
describe("validateFields", () => {
  it("should validate multiple fields successfully", () => {
    ...
    const errors = validateFields(fields, rules as any);
  ...
  it("should stop at first failing rule per field", () => {
    ...
    const rules = {
      name: [
        ...
        (v: string) =>
          validators.minLength(v, 3) || "Must be at least 3 characters",
      ],
    };
  ...
  it("should support custom error messages from rules", () => {
    ...
    const rules = {
      email: [
        (v: string) =>
          validators.email(v) || "Please enter a valid email address",
      ],
    };
