...
describe("api-tokens", () => {
  ...
  describe("createApiToken", () => {
    ...
    it("should allow verification of created token", async () => {
      ...
      const isValid = await verifyToken(
        result.token,
        result.tokenRecord!.tokenHash,
      );
      expect(isValid).toBe(true);
    });

    it("should support different expiration periods", async () => {
      const result7 = await createApiToken(
        "user-1",
        "a@test.com",
        "User",
        "Token7",
      ...
      const result30 = await createApiToken(
        "user-2",
        "b@test.com",
        "User",
        "Token30",
      ...
      const result90 = await createApiToken(
        "user-3",
        "c@test.com",
        "User",
        "Token90",
  ...
  describe("validateApiToken", () => {
    ...
    it("should reject non-existent token", async () => {
      const result = await validateApiToken(
        "mfc_nonexistent_token_12345678901234567890",
      );
      expect(result).toBeNull();
    });
    ...
    it("should reject revoked token", async () => {
      ...
      // Revoke the token
      await revokeApiToken(tokenRecord!._id.toString(), "user-123");
  ...
  describe("revokeApiToken", () => {
    it("should revoke an owned token", async () => {
      ...
      const db = getMockDatabase();
      const tokensCollection = db.collection("apitokens");
      const revokedToken = await tokensCollection.findOne({
        _id: tokenRecord!._id,
      });
  ...
  describe("listApiTokens", () => {
    ...
    it("should include both active and revoked tokens", async () => {
      ...
      // Revoke first token
      await revokeApiToken(token1!._id.toString(), "user-123");
  ...
  describe("token lifecycle integration", () => {
    ...
    it("should maintain security throughout lifecycle", async () => {
      ...
      const invalidResult = await validateApiToken(
        "mfc_wrong_token_12345678901234567890",
      );
