...
describe('GET /api/modules/[name]/resolve/[tag]', () => {
  ...
  it('should include metadata in response', async () => {
    ...
    expect(data.metadata.federation.exposes).toEqual(['./Component', './Header'])
    expect(data.metadata.federation.shared).toEqual({
      react: { version: '19.0.0', singleton: true },
    })
    expect(data.metadata.assets.remoteEntry).toBeDefined()
  })
