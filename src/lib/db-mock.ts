...
import type { MongoQuery, MongoUpdate, MongoSortSpec, InsertResult, UpdateResult, DeleteResult } from '@/types/database'
...
class MockCollection<T = Record<string, unknown>> {
  ...
  async findOne(query: MongoQuery): Promise<(T & { _id: string; createdAt: Date; updatedAt: Date }) | null> {
    ...
    for (const item of this.items.values()) {
      if (this.matches(item, query)) {
        return { ...item } as T & { _id: string; createdAt: Date; updatedAt: Date }
      }
    }
    return null
  }

  async find(query: MongoQuery = {}): Promise<(T & { _id: string; createdAt: Date; updatedAt: Date })[]> {
    await this.db.waitForInit()
    const results: (T & { _id: string; createdAt: Date; updatedAt: Date })[] = []
    for (const item of this.items.values()) {
      if (this.matches(item, query)) {
        results.push({ ...item } as T & { _id: string; createdAt: Date; updatedAt: Date })
      }
    }
    return results
  }
  ...
  private matches(item: Record<string, unknown>, query: MongoQuery): boolean {
    ...
    for (const [key, value] of Object.entries(query)) {
      ...
      if (key === '$or') {
        ...
        const orMatches = value.some((subQuery: MongoQuery) => this.matches(item, subQuery))
      ...
      if (key.includes('.')) {
        ...
        if ('$in' in operator) {
          if (!Array.isArray(operator.$in) || !operator.$in.includes(item[key])) return false
  ...
  private applyUpdate(item: Record<string, unknown>, update: MongoUpdate<T>): Record<string, unknown> {
    const result = { ...item }
