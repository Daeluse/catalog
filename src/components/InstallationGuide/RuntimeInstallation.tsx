    ...
    <div className="space-y-6">
      <div>
        <p className="mb-4 text-sm text-zinc-700">
          Use Module Federation runtime to dynamically load remotes at runtime.
          This approach doesn&apos;t require configuring remotes during build.
        </p>
      </div>
...
const remoteModule = await loadRemote('${moduleName}', {
  entry: '${remoteEntryUrl}'
          ...
          <button
            onClick={() =>
              copyToClipboard(`import { init, loadRemote } from '@module-federation/enhanced/runtime';
...
function MyComponent() {
  ...
  return RemoteComponent ? <RemoteComponent /> : <div>Loading...</div>;
                  ...
                  <button
                    onClick={() =>
                      copyToClipboard(`// Dynamic import
const { ${exposed.name || "Component"} } = await remoteModule.get('${exposed.path}');
  ...
  return RemoteComponent ? <RemoteComponent /> : <div>Loading...</div>;
      ...
      <div>
        <button
          ...
          className="flex items-center gap-2 text-sm font-medium text-zinc-900 hover:text-zinc-700"
        >
        ...
        {showAdvanced && (
          <div className="mt-4 space-y-4 rounded-md border border-zinc-200 bg-zinc-50 p-4">
            <p className="text-sm text-zinc-700">
              Instead of hardcoding version URLs, you can use the catalog&apos;s
              version resolution API to dynamically resolve version tags:
            </p>
            ...
            <div>
              ...
              <ul className="ml-4 list-disc space-y-1 text-sm text-zinc-700">
                <li>
                  <code className="rounded bg-zinc-200 px-1">latest</code> -
                  Most recent stable version
                </li>
                <li>
                  <code className="rounded bg-zinc-200 px-1">next</code> - Most
                  recent version (including prereleases)
                </li>
                <li>
                  <code className="rounded bg-zinc-200 px-1">^1.0.0</code> -
                  Caret ranges (compatible with 1.x.x)
                </li>
                <li>
                  <code className="rounded bg-zinc-200 px-1">~2.1.0</code> -
                  Tilde ranges (patch-level changes)
                </li>
                <li>
                  <code className="rounded bg-zinc-200 px-1">1.2.3</code> -
                  Specific version
                </li>
              </ul>
            </div>
...
const remoteModule = await loadRemote('${moduleName}', {
  entry: remoteEntry
              ...
              <button
                onClick={() =>
                  copyToClipboard(`// Resolve version tag to remoteEntry URL
const response = await fetch('/api/modules/${moduleName}/resolve/latest');
const { remoteEntry, resolvedVersion } = await response.json();
