...
import { VersionDocument, ModuleMaintainer } from '@/types/database'
...
async function deleteVersionAssets(versionDoc: VersionDocument): Promise<void> {
  const storage = getBlobStorageService()
  const blobPaths: string[] = []

  // Collect all blob paths from the version's assets
  if (versionDoc.assets) {
    const { remoteEntry, manifest, stats, types, chunks, documentation } = versionDoc.assets
    ...
    if (stats?.url) {
      const path = extractBlobPath(stats.url)
      if (path) blobPaths.push(path)
    }

    // Add optional assets
    if (types?.url) {
      const path = extractBlobPath(types.url)
      if (path) blobPaths.push(path)
    }
    if (documentation?.url) {
      const path = extractBlobPath(documentation.url)
      if (path) blobPaths.push(path)
    }
    ...
    if (chunks && Array.isArray(chunks)) {
      for (const chunk of chunks) {
        if (chunk?.url) {
          const path = extractBlobPath(chunk.url)
          if (path) blobPaths.push(path)
        }
      }
    }
...
// DELETE /api/modules/[name]/versions/[version] - Delete a specific version
export async function DELETE(
  ...
  try {
    ...
    if (!isAdmin && !isOwner && !isMaintainer) {
      return forbiddenResponse('You do not have permission to delete versions for this module')
    }
    ...
    if (module.latestVersion === version) {
      ...
      if (remainingVersions.length > 0) {
        ...
        remainingVersions.sort((a: VersionDocument, b: VersionDocument) => {
    ...
    return successResponse({ success: true, message: 'Version deleted successfully' })
  } catch (error) {
